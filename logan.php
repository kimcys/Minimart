...
if ($isLoggedIn) {
    $util->redirect("supplierdashboard.php");
}
...
if (! empty($_POST["login"])) {
    ...
    $username = $_POST["cashier_name"];
    $password = $_POST["cashier_password"];

    $user = $auth->getMemberByUsername($username);
    if (password_verify($password, $user[0]["cashier_password"])) {
        $isAuthenticated = true;
    }

    if ($isAuthenticated) {
        $_SESSION["cashier_id"] = $user[0]["cashier_id"];

        // Set Auth Cookies if 'Remember Me' checked
        if (! empty($_POST["remember"])) {
            setcookie("cashier_login", $username, $cookie_expiration_time);
...
<form action="" method="post" id="frmLogin">
  <h1 id="title" class="hidden"><span id="logo">Log In Cashier</h1>
    ...
    <div class="field-group">
        ...
        <div>
            <input name="cashier_name" type="text"
                value="<?php if(isset($_COOKIE["cashier_login"])) { echo $_COOKIE["cashier_login"]; } ?>"
                class="input-field">
        </div>
    </div>
    ...
    <div class="field-group">
        ...
        <div>
            <input name="cashier_password" type="password"
                value="<?php if(isset($_COOKIE["cashier_password"])) { echo $_COOKIE["cashier_password"]; } ?>"
                class="input-field">
        </div>
    </div>
    <div class="field-group">
        <div>
            <input type="checkbox" name="remember" id="remember"
                <?php if(isset($_COOKIE["cashier_login"])) { ?> checked
